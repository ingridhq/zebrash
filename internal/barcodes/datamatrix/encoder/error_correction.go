...
func ErrorCorrection_EncodeECC200(codewords []byte, symbolInfo *SymbolInfo) ([]byte, error) {
	if len(codewords) != symbolInfo.GetDataCapacity() {
		return nil, fmt.Errorf("the number of codewords does not match the selected symbol")
	}
...
func createECCBlock(codewords []byte, numECWords int) ([]byte, error) {
	...
	if table < 0 {
		return codewords, fmt.Errorf("illegal number of error correction codewords specified: %d", numECWords)
	}
	poly := factors[table]
