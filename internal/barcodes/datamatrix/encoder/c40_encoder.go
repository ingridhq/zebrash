...
func (enc *C40Encoder) getEncodingMode() int {
	return enc.encodingMode
}

func (enc *C40Encoder) encode(context *EncoderContext) error {
	//step C
	buffer := make([]byte, 0)
	for context.HasMoreCharacters() {
		...
		lastCharSize, buffer = enc.encodeChar(c, buffer)
		...
		if !context.HasMoreCharacters() {
			...
			if (len(buffer)%3) == 2 && available != 2 {
				lastCharSize, buffer, removed = enc.backtrackOneCharacter(context, buffer, removed, lastCharSize)
			}
...
func (enc *C40Encoder) backtrackOneCharacter(context *EncoderContext,
	...
	context.pos--
	c := context.GetCurrentChar()
	lastCharSize, removed = enc.encodeChar(c, removed)
...
func c40HandleEOD(context *EncoderContext, buffer []byte) error {
	...
	if rest == 2 {
		...
		return fmt.Errorf("unexpected case, please report")
	}
	context.SignalEncoderChange(HighLevelEncoder_ASCII_ENCODATION)
	return nil
}
