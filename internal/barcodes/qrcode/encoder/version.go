...
func Version_GetProvisionalVersionForDimension(dimension int) (*Version, error) {
	if dimension%4 != 1 {
		return nil, fmt.Errorf("dimengion = %v", dimension)
	}
...
func Version_GetVersionForNumber(versionNumber int) (*Version, error) {
	if versionNumber < 1 || versionNumber > 40 {
		return nil, fmt.Errorf("IllegalArgumentException: versionNumber = %d", versionNumber)
	}
...
func Version_decodeVersionInformation(versionBits int) (*Version, error) {
	...
	return nil, fmt.Errorf("we didn't find a close enough match 0x%x", versionBits)
}
