...
func (enc ASCIIEncoder) getEncodingMode() int {
	return HighLevelEncoder_ASCII_ENCODATION
}

func (enc ASCIIEncoder) encode(context *EncoderContext) error {
	//step B
	...
	if n >= 2 {
		...
		if newMode != enc.getEncodingMode() {
			...
			default:
				return fmt.Errorf("illegal mode: %v", newMode)
			}
...
func encodeASCIIDigits(digit1, digit2 byte) (byte, error) {
	...
	return 0, fmt.Errorf("not digits: %c%c", digit1, digit2)
}
