...
func (enc EdifactEncoder) getEncodingMode() int {
	return HighLevelEncoder_EDIFACT_ENCODATION
}

func (enc EdifactEncoder) encode(context *EncoderContext) error {
	//step F
	buffer := make([]byte, 0)
...
func edifactHandleEOD(context *EncoderContext, buffer []byte) error {
	...
	if count == 1 {
		...
		if e != nil {
			return fmt.Errorf("failed to update symbol info: %w", e)
		}
	...
	if count > 4 {
		return fmt.Errorf("count must not exceed 4, %v", count)
	}
	restChars := count - 1
...
func edifactEncodeChar(c byte, sb []byte) ([]byte, error) {
	if c >= ' ' && c <= '?' {
		...
		return sb, fmt.Errorf("illegal character: %v (0x%04x)", c, c)
	}
	return sb, nil
}
...
func edifactEncodeToCodewords(sb []byte) ([]byte, error) {
	len := len(sb)
	if len == 0 {
		return sb, fmt.Errorf("StringBuilder must not be empty")
	}
	c1 := int(sb[0])
	c2 := 0
	if len >= 2 {
