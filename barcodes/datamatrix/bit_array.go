...
func (b *BitArray) SetRange(start, end int) error {
	if end < start || start < 0 || end > b.size {
		return fmt.Errorf("invalid start/end values")
	}
	if end == start {
		return nil
	}
...
func (b *BitArray) IsRange(start, end int, value bool) (bool, error) {
	if end < start || start < 0 || end > b.size {
		return false, fmt.Errorf("invalid start/end values")
	}
	if end == start {
		return true, nil
	}
...
func (b *BitArray) AppendBits(value int, numBits int) error {
	if numBits < 0 || numBits > 32 {
		return fmt.Errorf("num bits must be between 0 and 32")
	}
	nextSize := b.size
...
func (b *BitArray) Xor(other *BitArray) error {
	if b.size != other.size {
		return fmt.Errorf("sizes don't match")
	}
